...
#include "tree.h"
#include "../File_reading/file_reading.hpp"
...
Tree_node* create_node(char* origin, Node_type type, Tree_node *left, Tree_node *right) {
...
#define Print_var_node(node)                                                                       \
        Print_code("node%p [label=\"{type: %s | var: %d}\",fillcolor=\"%s\",color=\"%s\"];\n",     \
                        node, Data_is_var, node->data.var_num, FILL__COLOR, FRAME_COLOR);
